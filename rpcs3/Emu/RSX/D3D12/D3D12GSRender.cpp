...
/*	if (!Ini.GSDumpDepthBuffer.GetValue())
	...
	case CELL_GCM_SURFACE_TARGET_MRT2:
		if (m_context_dma_color_a) rtt0 = writeColorBuffer(m_fbo->getRenderTargetTexture(0), downloadCommandList);
		if (m_context_dma_color_b) rtt1 = writeColorBuffer(m_fbo->getRenderTargetTexture(1), downloadCommandList);
		if (m_context_dma_color_c) rtt2 = writeColorBuffer(m_fbo->getRenderTargetTexture(2), downloadCommandList);
		break;
	...
	case CELL_GCM_SURFACE_TARGET_MRT3:
		...
		if (m_context_dma_color_b) rtt1 = writeColorBuffer(m_fbo->getRenderTargetTexture(1), downloadCommandList);
		if (m_context_dma_color_c) rtt2 = writeColorBuffer(m_fbo->getRenderTargetTexture(2), downloadCommandList);
		if (m_context_dma_color_d) rtt3 = writeColorBuffer(m_fbo->getRenderTargetTexture(3), downloadCommandList);
		break;
	}
	if (needTransfer)
	{
