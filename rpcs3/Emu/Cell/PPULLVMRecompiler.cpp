...
bool RecompilationEngine::isAddresCommited(u32 address) const
{
...
bool RecompilationEngine::AnalyseBlock(BlockEntry &functionData, u32 maxSize)
{
...
void RecompilationEngine::CompileBlock(BlockEntry & block_entry) {
	...
	for (auto compileResult : compileResult.first)
	{
		...
		if (It == m_address_to_function.end())
		{
			...
			if (!isAddresCommited(block.address / 4))
				commitAddress(block.address / 4);
		}

		FunctionCache[block.address / 4] = compileResult.second;
		...
		Log() << "Associating " << (u32*)block.address << " with ID " << m_currentId << "\n";
		m_currentId++;
